...
export default function App() {
          ...
          if (
            ...
            parsed.every(
              (h) =>
                ...
                Array.isArray(h.daysHistory) &&
                h.daysHistory.length > 0 &&
                h.capabilities &&
                typeof h.capabilities === "object"
            )
          ) {
            return parsed;
          }
  ...
  const activeHire =
    (newHires && newHires.find((h) => h.id === activeHireId)) ||
    (newHires && newHires[0]) ||
    initialCohort[0];
  ...
  const updateHireAndRecalculate = (
    ...
    setNewHires((prevHires) =>
      prevHires.map((hire) => {
        ...
        const existingRecordIndex = daysHistory.findIndex((d) => d.dayNumber === dayNum);
        const existingRecord: DayRecord =
          existingRecordIndex >= 0
            ? daysHistory[existingRecordIndex]
            : {
                dayNumber: dayNum,
                date: `Day ${dayNum}`,
                workSignal: {
        ...
        const previousRecord = daysHistory.find((d) => d.dayNumber === dayNum - 1);
