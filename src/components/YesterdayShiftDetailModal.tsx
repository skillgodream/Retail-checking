...
export const YesterdayShiftDetailModal: React.FC<YesterdayShiftDetailModalProps> = ({
  ...
  // LMS modules snapshot & Training Pillar
  ...
  const actualModulesDone = Math.min(targetModulesByYesterday, completedCount);
  ...
  const trainingScore = Math.min(
    100,
    Math.round(trainingCompletionPct * 0.7 + quizAvg * 0.3)
  );
  const ordersPct = Math.min(100, Math.round((ordersCompleted / targetOrders) * 100));

  // 4-Pillar Composite Daily Score
  ...
  // Good/Bad status based on composite score
  const isShiftGood =
    prevActionOutcome?.improved === "yes" || dailyPerformancePct >= 75;
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[9px] font-black px-2 py-0.5 rounded-full bg-violet-100 text-violet-800 border border-violet-200">
                  {dailyPerformancePct}% {isHindi ? "दैनिक स्कोर" : "Score"}
                </span>
              </div>
